  ...
  <div class="rk-trace-detail flex-v">
    <div class="rk-trace-detail-wrapper clear" v-if="current.endpointNames">
      ...
      <div class="mb-5 blue sm">
        ...
        <div class="link-hover cp inline-display" @click="handleClick(current.traceIds)">
          <svg class="icon vm grey ml-5">
      ...
      <a class="rk-btn mr-5 sm r" :class="{'ghost':displayMode !== 'table'}" @click="setDisplayMode('table')">
        <svg class="icon vm sm rk-trace-table_svg-icon">
          <use xlink:href="#table"></use>
        </svg>
        {{$t('table')}}</a>
      <a class="rk-btn mr-5 sm r" :class="{'ghost':displayMode !== 'tree'}" @click="setDisplayMode('tree')">
        <svg class="icon vm sm">
          <use xlink:href="#issue-child"></use>
        </svg>
        {{$t('tree')}}</a>
      <a class="rk-btn mr-5 sm r" :class="{'ghost':displayMode !== 'list'}" @click="setDisplayMode('list')">
        <svg class="icon vm sm">
